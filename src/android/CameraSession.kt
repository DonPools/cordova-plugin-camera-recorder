...
class CameraSession(
    ...
    fun getConfiguration(): JSONArray? {
        val sizesArr = JSONArray()
        for (itemSize in supportSizes) { 
            val sizeList = JSONObject() 
            sizeList.put("width", itemSize.getWidth())
            sizeList.put("height", itemSize.getHeight())
            sizesArr.put(sizeList)
        }
        return sizesArr
    }
